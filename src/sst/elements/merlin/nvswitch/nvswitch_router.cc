...
#include "nvswitch/nvswitch_router.h"
...
int nvswitch_router::num_routers = 0;
int nvswitch_router::print_debug = 0;
...
static std::string getLogicalGroupParam(const Params& params, Topology* topo, int port,
    ...
    if ( value == "" ) {
        ...
        if ( value == "" ) {
            // Abort
            merlin_abort.fatal(CALL_INFO, -1, "nvswitch_router requires %s to be specified\n", param.c_str());
        }
    }
    return value;
}
...
nvswitch_router::~nvswitch_router()
{
    delete [] in_port_busy;
...
nvswitch_router::nvswitch_router(ComponentId_t cid, Params& params) :
    Router(cid),
    num_vcs(-1),
    output(getSimulationOutput())
{
    ...
    if ( id == -1 ) {
        merlin_abort.fatal(CALL_INFO, -1, "nvswitch_router requires id to be specified\n");
    }

    num_ports = params.find<int>("num_ports",-1);
    if ( num_ports == -1 ) {
        merlin_abort.fatal(CALL_INFO, -1, "nvswitch_router requires num_ports to be specified\n");
    }
    ...
    if ( !topo ) {
        merlin_abort.fatal(CALL_INFO_LONG, 1, "nvswitch_router requires topology to be specified in input file\n");
    }
    ...
    if ( flit_size_s == "" ) {
        merlin_abort.fatal(CALL_INFO, -1, "nvswitch_router requires flit_size to be specified\n");
        abort();
    }
    ...
    if ( xbar_bw_s == "" ) {
        merlin_abort.fatal(CALL_INFO, -1, "nvswitch_router requires xbar_bw to be specified\n");
    }
    ...
    my_clock_handler = new Clock::Handler2<nvswitch_router,&nvswitch_router::clock_handler>(this);
    ...
    if ( xbar_bw_ua < link_bw  ) {
        merlin_abort.fatal(CALL_INFO_LONG,1,"ERROR: nvswitch_router requires xbar_bw to be greater than or equal to link_bw\n"
              "  xbar_bw = %s, link_bw = %s\n",
...
void
nvswitch_router::sigHandler(int signal)
{
    print_debug = num_routers * 5;
}

void
nvswitch_router::dumpState(std::ostream& stream)
{
...
void
nvswitch_router::printStatus(Output& out)
{
...
bool
nvswitch_router::clock_handler(Cycle_t cycle)
{
...
void
nvswitch_router::init(unsigned int phase)
{
    for ( int i = 0; i < num_ports; i++ ) {
        ports[i]->init(phase);
        Event *ev = NULL;
...
void
nvswitch_router::complete(unsigned int phase)
{
    for ( int i = 0; i < num_ports; i++ ) {
...
void
nvswitch_router::sendCtrlEvent(CtrlRtrEvent* ev, int port) {
    if ( port == -1 ) {
...
void
nvswitch_router::recvCtrlEvent(int port, CtrlRtrEvent* ev) {
...
void
nvswitch_router::reportIncomingEvent(internal_router_event* ev)
{
